<x-app-layout>
    <div class="container bg-white p-7">
        ...
        <div class="font-semibold grid grid-cols-1 md:grid-cols-3 gap-y-2 gap-x-2 py-3">
            <div>Authored by :<span class="font-normal"> {{$article->author_name}}</span></div>
            <div>Views :<span class="font-normal"> {{$article->views}}</span></div>
            <div>Section :<span class="font-normal"> test</span></div>
            <div>Created :<span class="font-normal"> {{\Carbon\Carbon::parse($article->created_at)->diffForHumans()}}</span></div>
            <div>Rating :<span class="font-normal"> </span></div>
        </div>
